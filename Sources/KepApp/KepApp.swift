...
/// misses `Contents/Resources/Kep_KepApp.bundle` when the binary is launched
/// directly (not via `open`), which trapped the whole app. We probe the real
/// locations and fall back to `.main`.
let kepLocalizationBundle: Bundle = {
    let name = "Kep_KepApp.bundle"
...
@inline(__always) func L(_ key: String.LocalizationValue) -> String {
    return String(localized: key, bundle: kepLocalizationBundle)
}
...
final class KepAppDelegate: NSObject, NSApplicationDelegate {
...
@main
struct KepApp: App {
    @NSApplicationDelegateAdaptor(KepAppDelegate.self) var appDelegate
    ...
    var body: some Scene {
        ...
        // Do not derive the window minimum from the live split-view widths.
        // Doing so lets widening the inspector increase the window's minimum
        // width until AppKit pushes the opposite edge off-screen.
        ...
        .commands {
            ...
            CommandMenu(L("menu.view.theme")) {
                ...
                Button(session.sidebarVisible ? L("menu.view.hide_sidebar") : L("menu.view.show_sidebar")) {
                    withoutAnimation { session.sidebarVisible.toggle() }
                }
            ...
            CommandMenu(L("menu.ai")) {
                Button("Assistant") {
                    withoutAnimation { session.outlineOpen = true }
            ...
            CommandMenu("Window") {
                ...
                Button(L(session.outlineOpen ? "menu.window.hide_outline" : "menu.window.show_outline")) {
                    withoutAnimation { session.outlineOpen.toggle() }
                }
...
final class AppSession {
    ...
    init() {
        ...
        // another app, switching to another Kep window, etc.
    ...
    // Sources/Kep/Session/AppSession+*.swift extensions:
