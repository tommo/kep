...
final class NotebookKernelStore {
    ...
    private var kernels: [String: KepNotebookKernel] = [:]
    ...
    func kernel(for url: URL?, restart: Bool, build: () -> KepNotebookKernel?) -> KepNotebookKernel? {
        let k = key(url)
...
/// KepScript Lua kernel. Lives in the app target (which imports both
/// KepMarkdown and KepScript) so KepMarkdown stays free of a scripting
...
extension AppSession {
    ...
    @MainActor
    private func buildNotebookKernel() -> KepNotebookKernel? {
        let (files, corpus) = workspaceCorpus()
        guard let kernel = try? KepNotebookKernel(map: MindMap(), corpus: corpus, allFiles: files) else { return nil }
    ...
    ///   1. global  `~/Library/Application Support/Kep/notebook.lua`
    ...
    @MainActor
    private func loadNotebookLibraries(into kernel: KepNotebookKernel) {
        var urls = [KepCore.applicationSupportURL.appendingPathComponent("notebook.lua")]
    ...
    func runNotebookAgent(_ question: String, context: String, in ctx: NotebookRunContext, into sink: NotebookAgentSink) async {
        ...
        let tools = KepAgentTools(map: MindMap(), corpus: corpus, allFiles: files,
        ...
        let specs = KepAgentTools.descriptors.filter { allow.contains($0.name) }
