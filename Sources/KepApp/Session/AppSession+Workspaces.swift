...
extension AppSession {
    ...
    /// Start an inline rename of the workspace header to set its display alias.
    func beginRenameWorkspace(_ root: NodeData) {
        renamingNodeURL = root.url.standardizedFileURL
    }

    /// Set (or clear, with an empty string) the workspace's display alias and
    /// persist it. Updates the live tree node so the sidebar reflects it at once.
    func setWorkspaceAlias(_ root: NodeData, to alias: String?) {
        let mgr = WorkspaceManager.shared
        guard let updated = mgr.setAlias(alias, forWorkspaceAt: root.url) else { return }
        root.name = updated.name
        workspaces = mgr.list.projects
        workspaceRoots = workspaceRoots   // re-publish so the header re-renders
    }
