...
/// Help → About Kep sheet. Reads name + version + build from the
...
/// newer version is published. Pure URL logic — UI lives in KepApp's
/// "Check for Updates…" menu item.
public enum ReleaseChecker {
    /// Override at boot via UserDefaults (`kep.update.releasesURL`) for
    /// forks or staging environments.
    public static var releasesPageURL: String {
        UserDefaults.standard.string(forKey: "kep.update.releasesURL")
            ?? "https://github.com/kep-app/kep/releases"
    }
}
