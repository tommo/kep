...
/// inspector), plus the global error alert. Owned by `KepApp.body`.
struct ContentView: View {
    ...
    @AppStorage("kep.prefs.inspector.agentExpanded") private var agentExpanded = false
    /// Live text of the inspector property/tag query field.
    @State private var tagQuery = ""
    /// Deterministic right-pane geometry. This is deliberately outside SwiftUI's
    /// `.inspector`, whose private presentation transition resizes/clips content
    /// even when transactions disable animation.
    @State private var inspectorWidth: CGFloat = 280
    @State private var inspectorDragStartWidth: CGFloat?
    /// Height of the node-note editor pane below the outline (resizable). Was a
    /// VSplitView, but its NSSplitView bridge overlaid the section header and
    /// blocked the fold toggle — a plain VStack + drag handle folds reliably.
    @State private var noteAreaHeight: CGFloat = 220
    @State private var noteDragStartHeight: CGFloat?
    ...
    private var sidebarColumnVisibility: Binding<NavigationSplitViewVisibility> {
        Binding(
            ...
            set: { visibility in
                withoutAnimation {
                    session.sidebarVisible = (visibility != .detailOnly)
                }
            }
        )
    }
        ...
        Rectangle()
            ...
            .overlay {
                Color.clear
                    ...
                    // Show the ↔ resize cursor while hovering the handle.
                    .onHover { inside in
                        if inside { NSCursor.resizeLeftRight.push() }
                        else { NSCursor.pop() }
                    }
                    .gesture(
                        // Read the drag in GLOBAL space. This divider sits just
                        // left of the inspector and the NavigationSplitView beside
                        // it is `maxWidth: .infinity`, so as `inspectorWidth`
                        // changes the divider's own frame shifts — moving the
                        // gesture's LOCAL origin mid-drag and feeding back into
                        // `translation` (resize → divider moves → translation is
                        // off → resize again). That feedback loop is the jitter.
                        // Global coordinates don't move with the divider.
                        DragGesture(minimumDistance: 0, coordinateSpace: .global)
                            .onChanged { value in
                                ...
                                inspectorWidth = min(460, max(200, start - value.translation.width))
                            }
    ...
    static let agentSystemPrompt =
        "You are Kep's assistant for the user's entire knowledge base — multiple mind maps and "
    ...
    private var outlineInspector: some View {
        VStack(spacing: 0) {
            ...
            // view uses, bound to the selected node's content (its Note). Sits
            // below the outline in a resizable pane (drag the handle to size).
            if session.selectedNodeProperties != nil {
                ...
                .frame(height: noteAreaHeight)
            }
        }
    }

    /// Drag handle between the outline list and the node-note editor.
    private var noteDragHandle: some View {
        Rectangle()
            .fill(Color(nsColor: .separatorColor))
            .frame(height: 1)
            .overlay {
                Color.clear
                    ...
                    .gesture(
                        DragGesture(minimumDistance: 0, coordinateSpace: .global)
                            .onChanged { value in
                                let start = noteDragStartHeight ?? noteAreaHeight
                                if noteDragStartHeight == nil { noteDragStartHeight = start }
                                noteAreaHeight = min(520, max(120, start - value.translation.height))
                            }
                            .onEnded { _ in noteDragStartHeight = nil }
                    )
            }
    }
...
final class WindowWidthKeeper {
    ...
    /// Bind to the document window. Pane layout is deliberately not allowed to
    /// mutate the window frame here: doing so during NavigationSplitView updates
    /// causes the inspector's inner rect to shift when the left sidebar toggles.
    func attach(_ w: NSWindow?) {
        if let w { window = w }
    }
}

/// Hands the host `NSWindow` to the document command scope.
