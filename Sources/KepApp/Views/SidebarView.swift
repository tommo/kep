...
struct SidebarView: View {
    ...
    private func workspaceHeaderRow(_ root: NodeData) -> some View {
        HStack(spacing: 4) {
            if session.renamingNodeURL == root.url.standardizedFileURL {
                // Inline alias editor — initial value is the current display name.
                Image(systemName: "books.vertical.fill").foregroundStyle(Color.accentColor)
                InlineRenameField(
                    initial: root.name,
                    onCommit: { newName in
                        session.setWorkspaceAlias(root, to: newName)
        ...
        .contextMenu {
            ...
            Divider()
            Button(L("sidebar.menu.rename_workspace")) { session.beginRenameWorkspace(root) }
            if root.url.lastPathComponent != root.name {
                Button(L("sidebar.menu.reset_workspace_alias")) { session.setWorkspaceAlias(root, to: nil) }
            }
