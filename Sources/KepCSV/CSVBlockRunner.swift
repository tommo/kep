...
public enum CSVBlockRunner {
    ...
    public static func run(_ blocks: [CSVEvalBlock], over document: CSVDocument) -> [CSVBlockResult] {
        ...
        for block in blocks {
            ...
            do {
                ...
                results.append(CSVBlockResult(name: block.name, value: "", output: buffer,
    ...
    /// Turn a thrown Lua error into a short, human-readable line: drop the noisy
    /// `[string "..."]:N:` chunk prefix (which echoes the whole wrapped source)
    /// and report a SOURCE-relative line. Our wrapper prepends one header line
    /// (`name = (function()`), so the Lua line is off by one from what the user
    /// typed — we subtract it back.
    private static func readableError(_ error: Error) -> String {
        if let lua = error as? LuaError {
            switch lua {
            case .runtimeFailure(let f):
                return formatLua(message: f.message, luaLine: f.line)
            case .syntaxError(let m), .runtimeError(let m):
                return stripChunkPrefix(m)
            case .instructionLimitExceeded:
                return "stopped: possible infinite loop"
            default:
                return (lua.errorDescription ?? "Lua error").replacingOccurrences(of: "\n", with: " ")
            }
        }
        return error.localizedDescription.replacingOccurrences(of: "\n", with: " ")
    }

    private static func formatLua(message: String, luaLine: Int?) -> String {
        let msg = message.trimmingCharacters(in: .whitespacesAndNewlines)
        if let luaLine, luaLine > 1 { return "line \(luaLine - 1): \(msg)" }
        return msg
    }

    private static func stripChunkPrefix(_ raw: String) -> String {
        let oneLine = raw.replacingOccurrences(of: "\n", with: " ")
        let ns = oneLine as NSString
        let pattern = "^(?:\\[string \"[\\s\\S]*?\"\\]|[^:\\s]+):(\\d+):\\s*"
        guard let re = try? NSRegularExpression(pattern: pattern),
              let match = re.firstMatch(in: oneLine, range: NSRange(location: 0, length: ns.length))
        else { return oneLine }
        let line = Int(ns.substring(with: match.range(at: 1)))
        let rest = ns.substring(from: match.range.upperBound)
        return formatLua(message: rest, luaLine: line)
    }
