...
/// `kep-wiki:` scheme, so the Markdown renderer turns them into `<a>` anchors
...
public enum WikiLinkMarkdown {
    public static let scheme = "kep-wiki"

    /// Replace each cross-document `[[…]]` with `[display](kep-wiki:Target#Heading)`.
    ...
    /// Decode a `kep-wiki:` URL string back into (target, heading?). Used by
