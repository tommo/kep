...
extension MindMapView {
    ...
    public override func keyDown(with event: NSEvent) {
        ...
        if let direction = Self.arrowKeyDirections[chars] {
            // ⌥+arrow pans the canvas (the graph scrolls under a fixed cursor);
            // ⇧+arrow extends the selection; a bare arrow moves it.
            if isOption { scrollCanvas(direction) }
            else if isShift { extendSelection(direction) }
            else { move(direction) }
            return
        }
    ...
    /// ⌥+arrow: pan the canvas one step in `direction` without changing the
    /// selection. The canvas is flipped (smaller y is visually higher), so
    /// "up" decreases the clip origin's y. Clamped to the content bounds.
    func scrollCanvas(_ direction: Direction) {
        guard let scroll = enclosingScrollView else { return }
        let clip = scroll.contentView
        let step: CGFloat = 120
        var origin = clip.bounds.origin
        switch direction {
        case .up:    origin.y -= step
        case .down:  origin.y += step
        case .left:  origin.x -= step
        case .right: origin.x += step
        }
