...
extension MindMapView {
    ...
    @objc public func cut(_ sender: Any?) {
        ...
        copy(sender)
        // Land the cursor on the parent of the primary cut topic afterwards, so
        // the selection doesn't vanish (parity with editors that select the
        // parent on cut). Captured before removal — `.parent` clears on remove.
        let primary = (selectedElement?.topic).flatMap { t in
            topics.contains(where: { $0 === t }) ? t : nil
        } ?? topics.first!
        let parent = primary.parent
        groupedUndo(name: topics.count > 1 ? "Cut Topics" : "Cut Topic") {
            for topic in topics { undoableRemove(topic) }
        }
        if let parent, let el = element(forTopic: parent) {
            selectElement(el)
        }
    }
    ...
    ///  1. kep topic-subtree → reparent the decoded subtree under the
    ...
    @objc public func paste(_ sender: Any?) {
        ...
        // kep-native subtree + image branches still win when both
