...
public struct PlantUMLEditor: NSViewRepresentable {
    ...
    public final class Coordinator: NSObject, NSTextViewDelegate, WKNavigationDelegate {
        ...
        func handlePreviewMenuAction(_ action: PreviewMenuAction) {
            ...
            case .copyHTML, .exportHTML, .exportPDF: break   // markdown-only
            }
        }
        ...
        /// Intercept `kep-src:<entity>` link clicks from the rendered diagram
        ...
        public func webView(_ webView: WKWebView,
            ...
            guard let url = navigationAction.request.url, url.scheme == "kep-src" else {
                decisionHandler(.allow); return
            }
            decisionHandler(.cancel)
            let raw = String(url.absoluteString.dropFirst("kep-src:".count))
