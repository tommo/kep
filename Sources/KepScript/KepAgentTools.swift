...
/// reference type so the value-type `KepAgentTools` can append to it.
public final class AgentToolEffects {
    ...
    /// Host-provided CSV cell access (the spreadsheet model lives in KepCSV,
    /// which depends on KepScript — so the app injects these to avoid a cycle).
    ...
    /// app/KepMarkdown layer). `notebookAddProse` appends a prose cell;
...
/// specs as `ToolSpec`s.
///
/// Tools are split across composable group files (`KepAgentTools+*.swift`),
...
public struct KepAgentTools {
    ...
    static let coreDescriptors: [(name: String, description: String, parametersJSON: String)] = [
        ...
        ("run_lua", "Run a Lua script against the mind map via the `kep` API; returns its result.",
    ...
    func handleCore(_ name: String, _ a: ToolArgs) -> String? {
        ...
        case "run_lua":
            ...
            let result = KepScriptRunner.run(script, on: map, corpus: corpus, allFiles: allFiles)
