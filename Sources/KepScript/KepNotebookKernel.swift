...
/// output is captured (KepLuaAPI has no print). One kernel == one notebook
/// run session; make a fresh kernel for an isolated single-cell run.
public final class KepNotebookKernel {
    ...
    public init(map: MindMap,
        ...
        engine.register("print") { [weak self] args in
            self?.printBuffer.append(args.map(KepScriptRunner.describe).joined(separator: "\t"))
            return .nil
        }
        try KepLuaAPI(map: map, corpus: corpus, allFiles: allFiles).install(on: engine)
    ...
    /// `kep`/`nb` API is set up, so it can define new globals or extend `kep`
    /// (e.g. `function kep.wordcount(s) … end`). Returns an error string if the
    ...
    public func run(_ source: String) -> ScriptRunResult {
        ...
        do {
            ...
            let ret = KepScriptRunner.describe(value)
            ...
            let e = KepScriptRunner.cleanError(error)
