...
final class AppSession {
    ...
    /// The Go to Node query. Owned by the session (not the modal's @State) so a
    /// SwiftUI re-render that re-creates the modal can't silently reset it and
    /// leave the list unfiltered while the field still shows text.
