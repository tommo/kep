...
/// Pure-logic transforms for Tab / Shift-Tab indent + outdent. Operates
/// on the selected lines as one block so callers can wire NSTextView
/// `insertTab(_:)` / `insertBacktab(_:)` straight through.
public enum MarkdownIndent {

    /// One indent level. 2 spaces matches the auto-continue-list output
    /// (#116) so manually-indented lists render the same as the
    /// auto-extended ones.
