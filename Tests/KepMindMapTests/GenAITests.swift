...
final class LLMConfigStoreTests: XCTestCase {
    func testRoundTripsThroughJSONFile() throws {
        let tmp = FileManager.default.temporaryDirectory.appendingPathComponent("kep-llm-\(UUID())")
    ...
    func testActiveSelectionRoundTripsForModelPicker() throws {
        let tmp = FileManager.default.temporaryDirectory.appendingPathComponent("kep-llm-active-\(UUID())")
        try FileManager.default.createDirectory(at: tmp, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tmp) }
