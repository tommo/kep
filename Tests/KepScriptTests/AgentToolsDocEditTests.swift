...
final class AgentToolsDocEditTests: XCTestCase {
    ...
    private func makeTools(files: [URL] = []) -> (KepAgentTools, AgentToolEffects) {
    ...
    func testCreateDocumentNoWorkspace() {
        let map = MindMap(root: Topic(text: "Root"))
        let tools = KepAgentTools(map: map, workspaceRoot: nil)
    ...
    func testDescriptorsValidJSON() {
        for d in KepAgentTools.docEditDescriptors {
            let data = Data(d.parametersJSON.utf8)
            XCTAssertNoThrow(try JSONSerialization.jsonObject(with: data),
                             "invalid JSON schema for \(d.name)")
        }
        let names = Set(KepAgentTools.docEditDescriptors.map { $0.name })
