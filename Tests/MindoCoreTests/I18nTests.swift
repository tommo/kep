...
/// and Simplified Chinese strings files exist on disk inside the Mindo
/// executable's resource layout, and that they share the same set of keys.
///
/// The Mindo SPM executable target ships its `.strings` files via
/// `resources: [.process("Resources")]`. The processed bundle lands next to
/// the test executable as `Mindo_Mindo.bundle` (SPM convention) — we walk
...
final class I18nTests: XCTestCase {
    ...
    private func locateStringsFiles() -> ResourcePaths? {
        ...
        // up looking for `*Mindo.bundle/Contents/Resources/{lproj}`.
        ...
        for _ in 0..<6 {
            ...
            if let candidate = try? fm.contentsOfDirectory(at: url, includingPropertiesForKeys: nil)
                .first(where: { $0.lastPathComponent.hasSuffix("_Mindo.bundle") || $0.lastPathComponent == "Mindo.bundle" }) {
        ...
        var packageRoot = sourceFile
            .deletingLastPathComponent()  // MindoCoreTests
            .deletingLastPathComponent()  // Tests
            .deletingLastPathComponent()  // package root
        // If we're in a worktree symlink, normalize.
        packageRoot = packageRoot.standardizedFileURL
        let en = packageRoot.appendingPathComponent("Sources/Mindo/Resources/en.lproj/Localizable.strings")
        let zh = packageRoot.appendingPathComponent("Sources/Mindo/Resources/zh-Hans.lproj/Localizable.strings")
        if fm.fileExists(atPath: en.path), fm.fileExists(atPath: zh.path) {
            return ResourcePaths(en: en, zh: zh)
        }
        return nil
    ...
    func testStringsFilesExistAndContainExpectedKeys() throws {
        guard let paths = locateStringsFiles() else {
            throw XCTSkip("Mindo localized resources not present alongside tests")
        }
